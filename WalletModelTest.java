...
public class WalletModelTest {
	...
	public void test1() {
		...
		model.setItemsFlatList(null);

		Assert.assertThrows(IllegalStateException.class, () -> model.buildTreeFromFlatList());
	}
	...
	public void test2() {
		...
		WalletItem oneNode = new WalletItem(ItemType.category, "Daten");
		WalletItem twoNode = new WalletItem(ItemType.item, "Uni");
		WalletItem threeNode = new WalletItem(ItemType.item, "Bank");

		model.getItemsFlatList().add(oneNode);
		model.getItemsFlatList().add(twoNode);
		model.getItemsFlatList().add(threeNode);
		model.buildTreeFromFlatList();

		Assert.assertSame(oneNode, twoNode.getParent());
		Assert.assertSame(oneNode, threeNode.getParent());
		Assert.assertSame(twoNode, oneNode.getChildren().get(0));
		Assert.assertSame(threeNode, oneNode.getChildren().get(1));
	}
