...
public class WalletModel {
	...
	public void addItem(final WalletItem parentItem, final WalletItem newItem) {
		//das parentItem ist die root der Datenbank
		if (isRoot(parentItem)) {
			addItemToRoot(parentItem, newItem);

		//das parentItem ist nicht die root der Datenbank
		} else {

			//Berechnet den index an dem newItem der itemFlatList hinzugefügt werden soll
			int index = calculateIndex(parentItem);

			//Fügt newItem der itemFlatList an der Position von index hinzu
			addItemToItemsFlatList(newItem, index);

			//Fügt newItem der child-Liste des parentItem hinzu
			parentItem.addChild(newItem);
		}

		//setzt modified Attribut auf true und ruft Event Dispatcher auf
		setModified(true);
	}

	private void addItemToItemsFlatList(WalletItem newItem, int index) {
		if (index == itemsFlatList.size() - 1)
			//Fügt newItem der itemsFlatList auf der letzten Position hinzu
			itemsFlatList.add(newItem);
		else
			//Fügt newItem der itemsFlatList nach der Position von index hinzu
			itemsFlatList.add(index + 1, newItem);
	}

	private int calculateIndex(WalletItem parentItem) {
		List<WalletItem> childrenList = parentItem.getChildren();
		//Prüft, ob die child-Liste des parentItems nicht vorhanden oder leer ist
		if (childrenList == null || childrenList.size() == 0) {
			//setzt index auf die Position des parentItems in der itemFlatList
			return getItemIndex(parentItem);
		} else {
			//lastChildren ist das letzte Kind in der child-List von parentItem
			WalletItem lastChildren = childrenList.get(childrenList.size() - 1);
			//setzt index auf die Position des letzten Kindes in der itemFlatList
			return getItemIndex(lastChildren);

		}
	}

	private void addItemToRoot(WalletItem parentItem, WalletItem newItem) {
		//prüft ob newItem eine Category ist, da nur Categories children von root sein können
		if (newItem.getType() != ItemType.category)
			throw new RuntimeException("Can only add category items to the root.");
		//Fügt dem parentItem das newItem als child hinzu
		parentItem.addChild(newItem);
		//fügt das newItem der itemsFlatList des models hinzu
		itemsFlatList.add(newItem);
	}
